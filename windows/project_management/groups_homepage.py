...
class GroupsForm:
    def __init__(self, groups_list, group_selection_layout, workspace_window):
        self.groups_groupbox = QtWidgets.QGroupBox('Groups:')
        self.myform = QtWidgets.QFormLayout()
        for group in groups_list:
            button = QtWidgets.QPushButton(f'{group}')
            button.clicked.connect(lambda: workspace_window.load_group_window(group))
...
class GroupSelectionWidget(QtWidgets.QWidget):
    def __init__(self, workspace_window):
        ...
        workspace_window.groups_form = GroupsForm(list(workspace_window.groups_details.keys()), group_selection_layout, workspace_window)
